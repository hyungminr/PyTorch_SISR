...
class EDSR(nn.Module):
    ...
    def __init__(self, args=None, num_RB=16, num_feats=64, scale=2, kernel=3, padding=1, bias=True):
        ...
        layers = [nn.Linear(in_features=1, out_features=18), nn.Softmax(dim=1)]
    ...
    def forward(self, img):    
        ...
        # shallow + deep
        weights = self.mlp(torch.zeros((img.shape[0], 1), device=img.device))
        deep = torch.cat([d.unsqueeze(1) for d in x_deep], dim=1)
        wd = weights.view(-1, 18, 1, 1, 1) * deep
        x_feature = wd[:,0,:,:,:]
        for i in range(1,18):
            x_feature = x_feature + wd[:,i,:,:,:]
