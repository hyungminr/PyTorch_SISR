...
class ChannAtt(nn.Module):
    def __init__(self):
        ...
        layers = [nn.Conv2d(in_channels=64, out_channels=64, kernel_size=(2, 1), padding=0, bias=True)]
        self.std_avg = nn.Sequential(*layers)
...
class EDSR(nn.Module):
    ...
    def __init__(self, args=None, num_RB=16, num_feats=64, scale=2, kernel=3, padding=1, bias=True):
        ...
        self.feature_attention = FeatureAtt2()
    ...
    def forward(self, img):    
        ...
        x_feature = self.feature_attention(x_deep[0], x_deep[-1])
