...
class FeatureAtt(nn.Module):
    def __init__(self):
        ...
        layers = [nn.Conv2d(in_channels=64, out_channels=64, kernel_size=1, padding=0, bias=True)]
    ...
    def forward(self, x, y, z):
        ...
        py = self.pool(y)
        pz = self.pool(z)
        pf = torch.cat([px, py, pz], dim=2)
        pw = self.conv(pf)
        fea_map = torch.cat([x.unsqueeze(2), y.unsqueeze(2), z.unsqueeze(2)], dim=2)
        fea_map = fea_map * pw.unsqueeze(-1)
