...
class hmnet(nn.Module):
    ...
    def __init__(self, args=None, num_RB=16, num_feats=64, scale=4, kernel=3, padding=1, bias=True):
        ...
        layers += [nn.Conv2d(in_channels=3, out_channels=num_feats, kernel_size=kernel, padding=padding, bias=bias)]
        layers += [nn.Conv2d(in_channels=3, out_channels=num_feats, kernel_size=kernel, padding=padding, bias=bias)]
        self.heads_hf = nn.ModuleList(layers)
        ...
        layers  = [nn.Conv2d(in_channels=num_feats, out_channels=num_feats, kernel_size=kernel, padding=padding, bias=bias, stride=2)]
        layers += [nn.Conv2d(in_channels=num_feats, out_channels=num_feats, kernel_size=kernel, padding=padding, bias=bias, stride=2)]
        self.downConv_hf = nn.ModuleList(layers)
        ...
        layers  = [nn.Conv2d(in_channels=num_feats, out_channels=num_feats*4, kernel_size=kernel, padding=padding, bias=bias)]
        layers += [nn.Conv2d(in_channels=num_feats, out_channels=num_feats*4, kernel_size=kernel, padding=padding, bias=bias)]
        self.upConv_hf = nn.ModuleList(layers)
        ...
        layers += [nn.Conv2d(in_channels=num_feats, out_channels=3, kernel_size=kernel, padding=padding, bias=bias)]
        layers += [nn.Conv2d(in_channels=num_feats, out_channels=3, kernel_size=kernel, padding=padding, bias=bias)]
        self.tails_hf = nn.ModuleList(layers)
    ...
    def forward(self, img, img_hf):    
        ...
        imgx1_hf = self.sub_mean(img_hf)
        imgx2_hf = self.upx2_bicubic(imgx1_hf)
        imgx4_hf = self.upx4_bicubic(imgx1_hf)
        ...
        fea_shallow_x4_hf = self.heads_hf[0](imgx4_hf)
        fea_shallow_x2_hf = self.heads_hf[1](imgx2_hf)
        fea_shallow_x1_hf = self.heads_hf[2](imgx1_hf)
        
        fea_shallow_x2 = fea_shallow_x2 + self.downConv[0](fea_shallow_x4)
        fea_shallow_x1 = fea_shallow_x1 + self.downConv[1](fea_shallow_x2)
        
        fea_shallow_x2_hf = fea_shallow_x2_hf + self.downConv_hf[0](fea_shallow_x4_hf)
        fea_shallow_x1_hf = fea_shallow_x1_hf + self.downConv_hf[1](fea_shallow_x2_hf)
        ...
        fea_x1_hf = fea_shallow_x1_hf + fea_deep_x1_hf
        fea_x1_to_x2_hf = self.pixelShuffle(self.upConv_hf[0](fea_x1_hf))
        
        fea_x2 = self.feaAtt_x2(fea_shallow_x2, fea_deep_x2, fea_x1_to_x2, fea_mshf_x2, fea_shallow_x2_hf, fea_deep_x2_hf, fea_x1_to_x2_hf)
        fea_x2_to_x4 = self.pixelShuffle(self.upConv[1](fea_x2))
        
        fea_x2_hf = fea_shallow_x2_hf + fea_deep_x2_hf + fea_x1_to_x2_hf
        fea_x2_to_x4_hf = self.pixelShuffle(self.upConv_hf[1](fea_x2_hf))
        
        fea_x4 = self.feaAtt_x4(fea_shallow_x4, fea_deep_x4, fea_x2_to_x4, fea_mshf_x4, fea_shallow_x4_hf, fea_deep_x4_hf, fea_x2_to_x4_hf)        
        fea_x4_hf = fea_shallow_x4_hf + fea_deep_x4_hf + fea_x2_to_x4_hf
        ...
        up_x4_hf = self.tails_hf[0](fea_x4_hf)
        up_x2_hf = self.tails_hf[1](fea_x2_hf)
        up_x1_hf = self.tails_hf[2](fea_x1_hf)
        ...
        img_x4_hf = self.add_mean(up_x4_hf)
        img_x2_hf = self.add_mean(up_x2_hf)
        img_x1_hf = self.add_mean(up_x1_hf)
        
        return img_x4, img_x2, img_x1, img_x4_hf, img_x2_hf, img_x1_hf
