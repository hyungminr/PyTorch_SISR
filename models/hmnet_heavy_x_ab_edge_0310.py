...
class hmnet(nn.Module):
    ...
    def __init__(self, args=None, num_RB=16, num_feats=64, scale=4, kernel=3, padding=1, bias=True):
        ...
        # self.downx2_bicubic = nn.Upsample(scale_factor=1/2, mode='bicubic', align_corners=False)
        # self.downx4_bicubic = nn.Upsample(scale_factor=1/4, mode='bicubic', align_corners=False)
        ...
        blocks += [RG(num_RCAB=64, num_feats=num_feats, kernel=kernel, padding=padding, bias=bias)]
    ...
    def forward(self, img):    
        ...
        imgx4 = self.sub_mean(img)
        imgx2 = nn.functional.interpolate(imgx4, scale_factor=1/2, mode='bicubic', align_corners=True, recompute_scale_factor=True)
        imgx1 = nn.functional.interpolate(imgx4, scale_factor=1/4, mode='bicubic', align_corners=True, recompute_scale_factor=True)
