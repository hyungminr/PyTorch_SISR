...
def get_ref_images(ref_images, size=256):
    ...
    w = np.random.randint(w-size)
    h = np.random.randint(h-size)
    tensor = tensor[:,:,w:w+size,h:h+size]
    tensor2 = torch.load(img.replace('.down',''))
    tensor2 = tensor2[:,:,w*2:w*2+size*2,h*2:h*2+size*2]
...
def train(model, train_loader, test_loader, mode='EDSR_Baseline', save_image_every=50, save_model_every=10, test_model_every=1, epoch_start=0, num_epochs=1000, device=None, refresh=True, scale=2):
    ...
    ref_images = glob.glob('./ref_image/bin/*.down.pt')
