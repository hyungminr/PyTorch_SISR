...
def train(model, train_loader, test_loader, mode='EDSR_Baseline', save_image_every=50, save_model_every=10, test_model_every=1, epoch_start=0, num_epochs=1000, device=None, refresh=True, scale=2):
    ...
    pool2 = torch.nn.MaxPool2d(2).to(device)
    pool3 = torch.nn.MaxPool2d(3).to(device)
    pool4 = torch.nn.MaxPool2d(4).to(device)
    ...
    def criterion_(pred, target):
        loss = criterion(pool2(pred), pool2(target))
        loss += criterion(pool2(-pred), pool2(-target))
        loss += criterion(pool3(pred), pool3(target))
        loss += criterion(pool3(-pred), pool3(-target))
        loss += criterion(pool4(pred), pool4(target))
        loss += criterion(pool4(-pred), pool4(-target))
        return loss
        ...
        with tqdm(train_loader, desc=f'{mode} || Epoch {epoch+1}/{num_epochs}', position=0, leave=True) as pbar:
            ...
            for lr, hr, _ in pbar:
                ...
                if soft_mask:
                    ...
                    # training
                    loss = criterion_(sr * gmask, hr * gmask)
                    lossx2 = criterion_(srx2 * gmaskx2, hrx2 * gmaskx2)
                    lossx1 = criterion_(srx1 * gmaskx1, hrx1 * gmaskx1)
                else:
                    loss = criterion_(sr, hr)
                    lossx2 = criterion_(srx2, hrx2)
                    lossx1 = criterion_(srx1, hrx1) 
