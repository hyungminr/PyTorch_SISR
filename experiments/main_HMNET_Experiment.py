...
train_loader = get_loader(mode='train', batch_size=batch_size, height=192, width=192, scale_factor=4, augment=True)
test_loader = get_loader(mode='test', height=256, width=256, scale_factor=4)
trainer.train(model, train_loader, test_loader, mode=f'HMNET_x{scale_factor}_heavy_x1_denoise_ablation_edge', epoch_start=epoch_start, num_epochs=num_epochs, save_model_every=100, test_model_every=1, today=today)
...
train_loader = get_loader(mode='train', batch_size=batch_size, height=192, width=192, scale_factor=4, augment=True)
test_loader = get_loader(mode='test', height=256, width=256, scale_factor=4)
trainer.train(model, train_loader, test_loader, mode=f'HMNET_x{scale_factor}_heavy_x1_denoise_ablation_fea', epoch_start=epoch_start, num_epochs=num_epochs, save_model_every=100, test_model_every=1, today=today)
...
while num_epochs <= 200:
    ...
    train_loader = get_loader(mode='train', batch_size=batch_size, height=192, width=192, scale_factor=4, augment=True)
    test_loader = get_loader(mode='test', height=256, width=256, scale_factor=4)
    trainer.train(model, train_loader, test_loader, mode=f'HMNET_x{scale_factor}_heavy_x1_denoise_ablation_fea', epoch_start=epoch_start, num_epochs=num_epochs, save_model_every=100, test_model_every=1, today=today, refresh=False)
...
today = datetime.datetime.now().strftime('%Y.%m.%d')
today = '2021.02.24'
model.load_state_dict(torch.load('./weights/2021.02.17/HMNET_x4_Heavy_REDS_size_352/epoch_0120.pth'))

size = 192 - 192
while num_epochs <= 200:
    epoch_start += num_epochs
    size += 192
    if size > 512: num_epochs = 3000
    train_loader = get_loader(data='REDS', mode='train', batch_size=batch_size, height=size, width=size, scale_factor=4, augment=True)
    test_loader = get_loader(data='REDS', mode='test', height=256, width=256, scale_factor=4)
    model = trainer.train(model, train_loader, test_loader, mode=f'HMNET_x{scale_factor}_Heavy_REDS_size_{size}', epoch_start=epoch_start, num_epochs=num_epochs, save_model_every=1, test_model_every=1, today=today, refresh=False)
...
today = datetime.datetime.now().strftime('%Y.%m.%d')
today = '2021.02.18'
model.load_state_dict(torch.load('./weights/2021.02.18/HMNET_x4_Heavy_REDS_size_0/epoch_0022.pth'))

size = 0
num_epochs = 3000
train_loader = get_loader(data='REDS', mode='train', batch_size=batch_size, height=size, width=size, scale_factor=4, augment=True)
test_loader = get_loader(data='REDS', mode='test', height=256, width=256, scale_factor=4)
trainer.train(model, train_loader, test_loader, mode=f'HMNET_x{scale_factor}_Heavy_REDS_size_{size}_hf_loss', epoch_start=22, num_epochs=num_epochs, save_model_every=1, test_model_every=1, today=today, refresh=False)
...
today = datetime.datetime.now().strftime('%Y.%m.%d')
today = '2021.03.02'
model.load_state_dict(torch.load('./weights/2021.02.18/HMNET_x4_Heavy_REDS_size_0_hf_loss/epoch_0038.pth'))
...
size = 0
num_epochs = 3000
train_loader = get_loader(mode='train', batch_size=4, height=192, width=192, scale_factor=scale_factor, augment=True)
...
train_loader = get_loader(data='SIDD', mode='train', batch_size=batch_size, height=256, width=256, scale_factor=1, augment=True)
test_loader = get_loader(data='SIDD', mode='test', height=256, width=256, scale_factor=1)
trainer.train(model, train_loader, test_loader, mode=f'HMNET_x{scale_factor}_SIDD_ablation_fea', epoch_start=epoch_start, num_epochs=num_epochs, save_model_every=1, test_model_every=1, today=today)
...
model = hmnet(scale=scale_factor)
today = datetime.datetime.now().strftime('%Y.%m.%d')
model.load_state_dict(torch.load('./weights/2021.03.02/HMNET_x4_Heavy_no_fea_REDS_size_0/epoch_0027.pth'))
...
from models.hmnet_heavy_x1_ablation_fea import hmnet
from utils.data_loader import get_loader
import trainer_hmnet_REDS_jpeg_last as trainer
torch.manual_seed(0)
scale_factor = 4
