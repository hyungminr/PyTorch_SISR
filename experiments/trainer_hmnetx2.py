...
def train(model, train_loader, test_loader, mode='EDSR_Baseline', save_image_every=50, save_model_every=10, test_model_every=1, epoch_start=0, num_epochs=1000, device=None, refresh=True, scale=2, today=None):
    ...
    upx2_bicubic = nn.Upsample(scale_factor=2, mode='bicubic', align_corners=False)
    ...
    for epoch in range(epoch_start, epoch_start+num_epochs):
        ...
        with tqdm(train_loader, desc=f'{mode} || Epoch {epoch+1}/{num_epochs}', position=0, leave=True) as pbar:
            ...
            for lr, hr, _ in pbar:
                ...
                hrx1 = downx2_bicubic(hr)
                hrx4 = upx2_bicubic(hr)
                ...
                if soft_mask:
                    ...
                    gmaskx4 = upx2_bicubic(gmask)
                    gmaskx1 = downx2_bicubic(gmask)
                    
                    # training
                    loss = criterion(sr * gmask, hr * gmask)
                    lossx4 = criterion(srx4 * gmaskx4, hrx4 * gmaskx4)
                ...
                # training
                loss_tot = loss + 0.25 * lossx4 + 0.125 * lossx1
                optim.zero_grad()
                loss_tot.backward()
                optim.step()
                scheduler.step()
