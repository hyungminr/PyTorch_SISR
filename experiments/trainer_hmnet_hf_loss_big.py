...
def criterion_(pred, target, high_pass, criterion):

    predx0 = high_pass.head0.forward(pred)
    predx1 = high_pass.head1.forward(predx0)
    predx2 = high_pass.head2.forward(predx1)
    
    targetx0 = high_pass.head0.forward(target)
    targetx1 = high_pass.head1.forward(targetx0)
    targetx2 = high_pass.head2.forward(targetx1)
    
    loss = criterion(predx0, targetx0)
    loss += criterion(predx1, targetx1)
    loss += criterion(predx2, targetx2)
    return loss
...
def train(model, train_loader, test_loader, mode='EDSR_Baseline', save_image_every=50, save_model_every=10, test_model_every=1, epoch_start=0, num_epochs=1000, device=None, refresh=True, scale=2):
    ...
    high_pass.load_state_dict(torch.load('./weights/2021.02.02/HMNET_x4_High_Pass_Filter_lite_v2/epoch_0100.pth'))
        ...
        with tqdm(train_loader, desc=f'{mode} || Epoch {epoch+1}/{num_epochs}', position=0, leave=True) as pbar:
            ...
            for lr, hr, _ in pbar:
                ...
                if soft_mask:
                    ...
                    loss_img = loss + 0.25 * lossx2 + 0.125 * lossx1
                    
                    loss_hf = criterion_(sr * gmask, hr * gmask, high_pass, criterion)
                    loss_hfx2 = criterion_(srx2 * gmaskx2, hrx2 * gmaskx2, high_pass, criterion)
                    loss_hfx1 = criterion_(srx1 * gmaskx1, hrx1 * gmaskx1, high_pass, criterion)                    
                    loss_hf = loss_hf + 0.25 * loss_hfx2 + 0.125 * loss_hfx1
                    ...
                    loss_hf = criterion_(sr, hr, high_pass, criterion)
                    loss_hfx2 = criterion_(srx2, hrx2, high_pass, criterion)
                    loss_hfx1 = criterion_(srx1, hrx1, high_pass, criterion)               
                    loss_hf = loss_hf + 0.25 * loss_hfx2 + 0.125 * loss_hfx1
                
                
                # training
                loss_tot = loss_img + loss_hf
                optim.zero_grad()
                loss_tot.backward()
                optim.step()
                scheduler.step()
                ...
                pfix['Loss HF'] = f'{loss_hf.item():.4f}'
