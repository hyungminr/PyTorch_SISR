    ...
    for epoch in range(epoch_start, epoch_start+num_epochs):
            ...
            with torch.no_grad():
                with tqdm(test_loader, desc=f'{mode} || Warming Up || Test Epoch {epoch}/{num_epochs}', position=0, leave=True) as pbar_test:
                    ...
                    for lr, hr, fname in pbar_test:
                        ...
                        lr_input = torch.cat((lr, lr_high, lr_low), dim=1)
                        
                        sr, features = model(lr_input)
                        sr, sr_high, sr_low = sr[:,:3,:,:], sr[:,3:6,:,:], sr[:,6:,:,:]
        ...
        with tqdm(train_loader, desc=f'{mode} || Epoch {epoch+1}/{num_epochs}', position=0, leave=True) as pbar:
            ...
            for lr, hr, _ in pbar:
                ...
                lr_high, lr_low = pass_filter(lr)
                hr_high, hr_low = pass_filter(hr)
                lr_input = torch.cat((lr, lr_high, lr_low), dim=1)
                
                # prediction
                sr, freqs = model(lr_input)
                sr, sr_high, sr_low = sr[:,:3,:,:], sr[:,3:6,:,:], sr[:,6:,:,:]
                ...
                loss_freq = 0.2*criterion(sr_high, hr_high) + 0.1*criterion(sr_low, hr_low) 
                loss_tot = loss + loss_freq
                optim.zero_grad()
                loss_tot.backward()
                optim.step()
                scheduler.step()
