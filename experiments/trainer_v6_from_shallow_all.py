    ...
    for epoch in range(num_epochs):
        ...
        with tqdm(train_loader, desc=f'Mode: {mode} || Epoch {epoch+1}/{num_epochs}', position=0, leave=True) as pbar:
            ...
            for lr, hr, _ in pbar:
                ...
                # training
                loss = criterion(hr, sr[0])
                loss_shallow = 0
                for i, sr_ in enumerate(sr[1:]):
                    loss_shallow += 2**(-i-1) * criterion(hr, sr_)
                loss_tot = loss + loss_shallow
                optim.zero_grad()
                loss_tot.backward()
                optim.step()
                scheduler.step()
                ...
                pfix['Loss Shallow'] = f'{loss_shallow.item():.4f}'
