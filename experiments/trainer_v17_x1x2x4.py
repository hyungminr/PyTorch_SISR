...
def train(model, train_loader, test_loader, mode='EDSR_Baseline', save_image_every=50, save_model_every=10, test_model_every=1, num_epochs=1000, device=None, refresh=True):
    ...
    Down_x2 = nn.MaxPool2d(2).to(device)
    Down_x4 = nn.MaxPool2d(4).to(device)
    ...
    for epoch in range(num_epochs):
        ...
        if epoch == pretrained_epoch:
           ...
           model.load_state_dict(torch.load(f'./weights/2021.01.19/EDSR_x2_Baseline/epoch_{pretrained_epoch:04d}.pth'))
        if epoch == 0:
        ...
        with tqdm(train_loader, desc=f'Mode: {mode} || Epoch {epoch+1}/{num_epochs}', position=0, leave=True) as pbar:
            ...
            for lr, hr, _ in pbar:
                ...
                loss_dr = 0.2 * criterion(features[0], dr_x2)
                loss_dr += 0.1 * criterion(features[1], dr_x1)
                
                gmsd = GMSD(hr, sr)
                
                # training
                loss = criterion(sr, hr)
                loss_tot = loss + loss_dr
                optim.zero_grad()
                loss_tot.backward()
                optim.step()
                scheduler.step()
