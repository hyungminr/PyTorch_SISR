...
from models.common import VGG_Extractor
    ...
    for epoch in range(epoch_start, epoch_start+num_epochs):
        ...
        with tqdm(train_loader, desc=f'{mode} || Epoch {epoch+1}/{num_epochs}', position=0, leave=True) as pbar:
            ...
            for lr, hr, _ in pbar:
                ...
                sr_vgg = vgg(sr * (1.2-gmask))
                hr_vgg = vgg(hr * (1.2-gmask))
                loss_vgg = 0
                for sr_fea, hr_fea in zip(sr_vgg, hr_vgg):
                    loss_vgg += criterion(sr_fea, hr_fea)
                loss_vgg = 0.1 * loss_vgg
                
                loss_tot = loss + loss_vgg
                optim.zero_grad()
                loss_tot.backward()
                optim.step()
                scheduler.step()
                ...
                pfix['Loss vgg'] = f'{loss_vgg.item():.4f}'
